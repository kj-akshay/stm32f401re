...
/**
 * @brief	This function enables are disables the gpio clock
 *
...
void GPIO_Init(GPIO_Handle_t *pGPIOHandle){
	...
	else
	{
		...
		//2. configure the gpio port selection in SYSCG_EXTICR
	...
	//4. configure the outpt type
	temp = (pGPIOHandle->GPIO_PinConfig.GPIO_PinOPType << (pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber));
	pGPIOHandle->pGPIOx->OTYPER &= ~(0x1 << pGPIOHandle->GPIO_PinConfig.GPIO_PinNumber);
	pGPIOHandle->pGPIOx->OTYPER |= temp;
	temp = 0;

	//5.configure configure the alt functionality
...
{
	if(Value == GPIO_PIN_SET)		//if set write 1 to the output data reg at bit field corresponding to pin number
	{
		pGPIOx->ODR |= (1<< PinNumber);
	}
	else			//if set write 0 to the output data reg at bit field corresponding to pin number
	{
		pGPIOx->ODR &= ~(1<< PinNumber);
	}
}
